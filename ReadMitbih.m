...
%------ SPECIFY DATA ------------------------------------------------------
%------ 指定数据文件 -------------------------------------------------------
PATH= 'F:\mitbih';          % 指定数据的储存路径
HEADERFILE= '109.hea';                                                      % .hea 格式，头文件，可用记事本打开
ATRFILE= '109.atr';                                                         % .atr 格式，属性文件，数据格式为二进制数
DATAFILE='109.dat';                                                         % .dat 格式，ECG 数据
...
for k=1:nosig           % 读取每个通道信号的数据信息
    z= fgetl(fid1);
    A= sscanf(z, '%*s %d %d %d %d %d',[1,5]);   % %*s表示这里有若干字符，但不赋值，占位符
...
case 2
    ...
    TIME=(0:(SAMPLES2READ-1))/sfreq;          %转换过后的十进制数据存储在矩阵M中
case 1
    M( : , 1)= (M( : , 1)- zerovalue(1));
    M( : , 2)= (M( : , 2)- zerovalue(1));
    M=M';
...
%------ SAVE DATA ------------------------------------------------------
save ECGDATA M1 M2 ANNOTD TIME DATAFILE ATRTIMED nosig;

AimTime=[];
for k=1:length(ATRTIMED)
    if ANNOTD(k)==5
           AimTime=[round(AimTime);ATRTIMED(k)];
    end;
end;